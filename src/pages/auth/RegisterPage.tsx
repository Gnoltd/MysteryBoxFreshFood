...
export default function RegisterPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      navigate(result === 'new' ? '/auth/setup' : '/')
    } catch {
  ...
  return (
    <Card className="bg-slate-900 border-slate-800">
      <CardHeader>
        ...
        <CardDescription className="text-slate-400">{t('auth.joinMysteryBox')}</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-slate-300">{t('auth.displayName')}</Label>
        ...
        <p className="text-slate-400 text-sm text-center mt-4">
          {t('auth.haveAccount')} <Link to="/login" className="text-indigo-400 hover:underline">{t('auth.signIn')}</Link>
        </p>
      </CardContent>
    </Card>
  )
